   ...
   /*
    ...
    if(news==null){
		...
		if(login==1){
			...
			ActionContext.getContext().put("Sessionerror","用户名或密码不可为空！且要长度必须大于6");
		}else if(login==4){
			ActionContext.getContext().put("Sessionerror","两次输入密码不一致！");
			}else if(login==5){
				ActionContext.getContext().put("Sessionerror","密码修改成功！");
				
			}else{
				ActionContext.getContext().put("Sessionerror","头像上传失败！");
