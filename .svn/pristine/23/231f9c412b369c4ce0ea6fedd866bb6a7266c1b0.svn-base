...
<div class="content">
  ...
  <s:if test="#request.downloadId==1">教学教案</s:if>
  <s:if test="#request.downloadId==2">电子课件</s:if>
  <s:if test="#request.downloadId==3">重点难点</s:if>
    ...
    <div class="file_area">
    	...
    	<s:iterator id="gz" value="teachCaseList">
        		<li>
            	<div class="file_left"><a href="javascript:teachDialog('${gz.resourceURL}',${downloadId});" class="fileTest">${gz.resourceName}</a></div>
               
                <div class="file_right">${gz.uploadDate}
                
                 <s:a id="download" action="downloadResourceAction_resourceDownload?resourceId=%{id}&downloadId=%{downloadId}&courseid=%{courseid}">Download</s:a>
